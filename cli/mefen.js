...
if (!semver.satisfies(currentNodeVersion, minimumNodeVersion)) {
    ...
    }\nMefen requires Node version ${minimumNodeVersion
...
function requireCLI(preferLocal) {
    ...
    if (preferLocal) {
        try {
            const localCLI = require.resolve(path.join(process.cwd(), 'node_modules', 'generator-mefen', 'cli', 'cli.js'));
            if (__dirname !== path.dirname(localCLI)) {
                // load local version
                /* eslint-disable import/no-dynamic-require */
                logger.info('Using Mefen version installed locally in current project\'s node_modules');
                require(localCLI);
                return;
            }
        } catch (e) {
    ...
    // load global version
    logger.info('Using Mefen version installed globally');
    require('./cli');
    /* eslint-enable  */
}
